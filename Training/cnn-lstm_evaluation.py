...
# CNN-LSTM Model (Matching the training script's architecture)
class CNNLSTMModel(nn.Module):
    def __init__(self, input_channels=1, cnn_filters=8, lstm_hidden_dim=32, num_layers=1, num_classes=2, dropout=0.4):
        super(CNNLSTMModel, self).__init__()
        # CNN layers for feature extraction (same as CNN-BiLSTM)
        ...
        self.relu = nn.ReLU()
        # LSTM for temporal modeling (unidirectional)
        self.lstm = nn.LSTM(cnn_filters, lstm_hidden_dim, num_layers=num_layers, batch_first=True,
    ...
    def forward(self, x):
        ...
        # LSTM part (unidirectional)
        lstm_out, _ = self.lstm(x)  # [batch, 42, lstm_hidden_dim]
        x = lstm_out[:, -1, :]  # Take the last time step: [batch, lstm_hidden_dim]
        x = self.dropout(x)
...
def evaluate_model(model, test_loader, mapping, device):
    ...
    plt.title('ROC Curves for CNN-LSTM Model')
    plt.legend(loc='lower right')
    plt.grid(True)
    plt.savefig('cnn_lstm_roc_curve.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    model_path = os.path.join(save_dir, "mixed_audio_cnn_lstm.pth")

    # Verify model file exists
    if not os.path.exists(model_path):
        print(f"Error: Model file not found at {model_path}. Please run cnn_lstm_train.py first.")
        exit(1)
