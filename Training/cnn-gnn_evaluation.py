...
class GraphFeatureDataset(Dataset):
    def __init__(self, features_path, labels_path, mapping_path):
        ...
        # Debug: Print the structure of the first mapping entry
        print(f"Sample mapping entry: {self.mapping[0]}")

    def __len__(self):
        return len(self.labels)

    def __getitem__(self, idx):
        # Ensure mapping[idx] is a tuple of (file_name, segment_idx)
        file_name, segment_idx = self.mapping[idx]  # Unpack directly here
        return torch.FloatTensor(self.features[idx]), torch.LongTensor([self.labels[idx]]), (file_name, idx)
...
def evaluate_model(model, test_loader, device):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, labels, mapping) in enumerate(test_loader):
            ...
            # Debug: Print the mapping structure for the first batch
            if batch_idx == 0:
                print(f"Mapping structure for first batch: {mapping}")

            # Extract file names and indices from mapping
            file_names, indices = mapping  # mapping is a tuple (file_names, indices)
            indices = indices.cpu().numpy()  # Convert tensor to numpy array

            # Map predictions to files
            for pred, label, file_name, idx, prob in zip(batch_preds, batch_labels, file_names, indices, probabilities):
    ...
    plt.savefig('cnn_gnn_confusion_matrix.png')
    plt.close()
    ...
    plt.title('ROC Curves for CNN-GNN Model')
    plt.legend(loc='lower right')
    plt.grid(True)
    plt.savefig('cnn_gnn_roc_curve.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    model_path = os.path.join(save_dir, "mixed_audio_cnn_gnn.pth")

    # Verify model file exists
    if not os.path.exists(model_path):
        print(f"Error: Model file not found at {model_path}. Please run cnn_gnn_train.py first.")
        exit(1)
    ...
    unseen_dataset = GraphFeatureDataset(unseen_features_path, unseen_labels_path, unseen_mapping_path)
    ...
    evaluate_model(model, unseen_loader, device)